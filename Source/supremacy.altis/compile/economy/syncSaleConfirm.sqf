...
if (_item != "") then {
    ...
    if (_confirmed) then {
        ...
        if (_attachments) then {
            hint format["%1%2 sold for $%3!", (_item select 1), " (the attachment(s) where sold in the background)", _price];
        } else {
            hint format["%1 sold for $%2!", (_item select 1), _price];
        };
        ...
        hint format["Sale of the %1 failed!", (_item select 1)];
        player say3D "error";
    };
};
